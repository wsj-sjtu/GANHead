...
def generate_mesh(func, verts_ori, level_set=0, res_init=16, res_up=3):
    ...
    for comp in connected_comp:
        ...
        distance = sum((comp.centroid - np.zeros_like(comp.centroid))**2)
        if distance < max_d:
            max_d = distance
            center_comp = comp
    
    if center_comp.area < 0.5*max_comp.area:
        head_comp = max_comp
    else:
        head_comp = center_comp
        
    meshexport = head_comp

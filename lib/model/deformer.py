...
class ForwardDeformer(torch.nn.Module):
    ...
    def forward(self, xd, cond, tfs, mask=None, eval_mode=False):
        ...
        cond_ori = cond.copy()   # original cond (for self.flame.forward_pts)

        cond = { key:expand_cond(cond[key], xd) for key in cond}    # e.g.(16,100)->(16,2250,100)
        cond = mask_dict(cond, mask)   # e.g.(16,2250,100)->(36000,100)

        for key in cond:
            cond[key] = cond[key][:,None].expand(-1, self.n_init, -1).flatten(0, 1)     # e.g.(36000,100)->(180000,100)
            
        xd = xd[mask]

        xc_init = self.__init(xd, tfs, cond_ori)

        n_init = xc_init.shape[1]

        tfs = tfs[:,None].expand(-1, n_init, -1, -1, -1).flatten(0, 1)

        xc_opt, others = self.__search(xd, xc_init, cond, cond_ori, tfs, eval_mode=eval_mode)
        ...
        if not eval_mode:
            ...
            xd_opt = self.__forward_skinning_flame(xc_opt, cond, cond_ori, tfs)

            grad_inv = self.__gradient(xc_opt, cond, cond_ori, tfs).inverse()
    ...
    def query_weights(self, xc, cond, mask=None, val_pad=0):
        ...
        out, shapedirs, posedirs = self.__query_weights(xc, cond, warp=False)
        ...
        return out_full, shapedirs, posedirs

    def __init(self, xd, tfs, cond_ori):
        ...
        for i in self.init_bones:
            ...
            w[:, i] = 1
            xc_init.append(self.flame.inverse_skinning_pts(xd, w, tfs, cond_ori['shape']))

        xc_init = torch.stack(xc_init, dim=-2)

        return xc_init.reshape(n_point, len(self.init_bones), n_dim)

    def __search(self, xd, xc_init, cond, cond_ori, tfs, eval_mode=False):
        ...
        if not eval_mode:
            J_inv_init = self.__gradient(xc_init, cond, cond_ori, tfs).inverse()
        else:
            # # w, _, _ = self.query_weights(xc_init, cond)
            w, _, _ = self.__query_weights(xc_init, cond)
        ...
        def _func(xc_opt, mask):
            ...
            xc_opt = xc_opt.squeeze(-1) #.reshape(n_batch, n_point * n_init, n_dim)

            xd_opt = self.__forward_skinning_flame(xc_opt, cond, cond_ori, tfs, mask=mask)
    ...
    def __forward_skinning(self, xc, cond, tfs):
        ...
        """
        w, _, _ = self.query_weights(xc, cond)
    ...
    def __query_weights(self, xc, cond, warp=True):
        ...
        Returns:
            w (tensor): skinning weights. shape: [N, J]
            blendshapes: shape and pose bases. shape: [N, 258]  258=(50+36)*3
        """

        if warp:
            xc = self.__query_cano(xc, cond)
        
        w, blendshapes = self.lbs_network(xc, cond)

        w = self.soft_blend * w
        w = F.softmax(w, dim=-1)

        posedirs = blendshapes[:, :36*3].reshape(-1, 36, 3)   # (n, 36, 3)
        shapedirs = blendshapes[:, 36*3:].reshape(-1, 3, 50)   # (n, 3, 50)

        return w, shapedirs, posedirs

    def __gradient(self, xc, cond, cond_ori, tfs):
        """Get gradients df/dx
        ...
        xd = self.__forward_skinning_flame(xc, cond, cond_ori, tfs)
    ...
    def __forward_skinning_flame(self, xc, cond, cond_ori, tfs, mask=None):
        """Canonical point -> deformed point
        ...
        Returns:
            xd (tensor): deformed point. shape: [B, N, D]
        """
        w, shapedirs, posedirs = self.__query_weights(xc, cond)

        xd = self.flame.forward_pts(xc, w, tfs, cond_ori['shape'], cond_ori['exp'], cond_ori['pose_feature'], 
                                    shapedirs=shapedirs, posedirs=posedirs, mask=mask)

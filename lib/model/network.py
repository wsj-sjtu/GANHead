...
class ImplicitNetwork(nn.Module):
    def __init__(
            ...
            feat_cond_dim=0,
            feat_cond_layer=[],
            is_lbs_network=False,
            **kwargs
    ):
        super().__init__()

        self.is_lbs_network = is_lbs_network
        ...
        if is_lbs_network:
            n_exp = 50
            n_pose = 36
            self.blendshapes = nn.Linear(dims[-3], (n_exp+n_pose)*3)
            # initialize blendshapes to be zero
    ...
    def forward(self, input, cond, input_feat=None,  mask=None, return_feat=False, spatial_feat=False, val_pad=0, normalize=False):
        ...
        if input_dim == 3:
            ...
            ## add pose cond
            if spatial_feat:
                cond_new = { key:grid_sample_feat(cond[key], input) for key in cond if key in self.cond_names and key is not 'thetas'}
            else:
                cond_new = { key:expand_cond(cond[key], input) for key in cond if key in self.cond_names}
            cond_new['thetas'] = cond['thetas'][:,None,:].repeat(1,input.shape[1],1)

            cond = mask_dict(cond_new, mask)
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l==2 and self.is_lbs_network:    # for blendshape
        ...
        if self.is_lbs_network:
            if return_feat:
                return x, feat, blendshapes
            else:
                return x, blendshapes      # x:(n,5)  blendshapes:(n,258)
        else:       
            if return_feat:
                return x, feat
            else:

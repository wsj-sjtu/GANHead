...
class Renderer():
    ...
    def render_mesh_dict(self, mesh_dict, mode='npat'):
        ...
        with torch.no_grad():
            ...
            if 'y' in mode:
                normals_vis_x = normals_x* 0.5 + 0.5  # [0,1]
            ...
            if 't' in mode:    # denormalized abledo
                assert('color' in mesh_dict)
                color_vis = mesh_dict['color'][None] * 0.5 + 0.5    # (-1,1)->(0,1)
                mesh_teture = Meshes(mesh_dict['verts'][None], mesh_dict['faces'][None], textures=Textures(verts_rgb=color_vis))   # without shading
                image_color = self.renderer(mesh_teture)
                results.append(image_color)
...
def weights2colors(weights):
    ...
    color_mapping = {'orange': cmap.colors[7],
                    'blue': cmap.colors[1],
                    'green':cmap.colors[3],
                    'white': [1,1,1],
                    'red':cmap.colors[5],
    ...
    colors = np.stack(colors)[None]# [1x5x3]

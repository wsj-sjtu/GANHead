...
def lbs(betas, pose, v_template, shapedirs, posedirs, J_regressor, parents,
    ...
    return verts, J_transformed, T, W, A.view(batch_size, num_joints, 4,4), pose_feature
...
################# new for ganhead #####################
##-----------------------------------------------------
def blend_shapes_new(betas, shape_disps):
    ''' Calculates the per vertex displacement due to the blend shapes
    ...
    if len(shape_disps.shape) == 3:
        blend_shape = torch.einsum('ml,mkl->mk', [betas, shape_disps])    # betas:(6144,50)  shape_disps:(6144,3,50)
    return blend_shape


def pose_correctives(pose_feature, posedirs):
    # [num_points, 4*9] x [num_points, 4*9, 3] --> [num_points, 3]
    pose_correctives = torch.einsum('mi,mik->mk', [pose_feature, posedirs])   #(6144,36) (6144,36,3)
...
def inverse_pts(pnts_p, betas, transformations, pose_feature, shapedirs, posedirs, lbs_weights, dtype=torch.float32):
    ...
    pnts_c = pnts_c - blend_shapes_new(betas, shapedirs)
...
def forward_pts(pnts_c, betas, transformations, pose_feature, shapedirs, posedirs, lbs_weights, dtype=torch.float32, mask=None):
    ...
    # Add shape contribution
    pnts_shaped = pnts_c + blend_shapes_new(betas, shapedirs)
